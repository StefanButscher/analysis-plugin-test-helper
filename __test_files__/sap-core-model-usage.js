...
jQuery.sap.declare("js.core.getModel");
...
sap.ca.scfld.md.ComponentBase.extend("sap.ca.scfld.md.sample.Component", {
    ...
    onTitlePress : function (oEvent) {
        ...
        // For business Card
        var context = this.oBindingContext;
        var idApprover = context.getProperty("PersonalNumber");
        var oEmployee;
        var source = oEvent.oSource;

        var oContactContext = new sap.ui.model.Context(this.oDataModel, "/Contacts");
        var i18nModel = sap.ui.getCore().getModel("i18n");
        this.oDataModel._loadData("Contacts?&$filter=Id%20eq%20'"+idApprover+"'", null, jQuery.proxy(function (oData) {

            oEmployee = {
                fullname: oData.results[0].Name,
                imgurl: "",
                department: oData.results[0].Department,
                mobilephone: oData.results[0].Mobile,
                workphone: oData.results[0].Phone,
                email: oData.results[0].Email,
                companyname: oData.results[0].Company,
                companyaddress:oData.results[0].Address
            };

            var sTitle = this.oResourceBundle.getProperty("TRC_TITLE_EMPLOYE");
            //calls the business card
            sap.ui.getCore().getEventBus().publish("nav", "bc", {
                sTitle:sTitle,
                oData:oEmployee,
                oControl:source
            });

        }, this), null, oContactContext);
    },

//	,init : function() {
//		// router initialization and history handling is done by the scaffolding and must no b done here
//		// 	call sequence is: 1. scaffolding init (inclusive getRouter), 2. this init method, 3. router initialisation inclusive first navigation triggered by scaffolding
//	}
